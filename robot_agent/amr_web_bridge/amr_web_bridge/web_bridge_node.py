...
class WebBridgeNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('profile_moving_linear_threshold', 0.02)
        self.declare_parameter('profile_moving_angular_threshold', 0.05)
        ...
        self.blocked_start_recovery_window_seconds = min(
            120.0,
            max(
                30.0,
                float(self.get_parameter(
                    'blocked_start_recovery_window_seconds'
                ).value),
            ),
        )
        ...
        self.navigation_recovery_runner = NavigationRecoveryRunner()
        ...
        self.profile_moving_linear_threshold = max(
            0.001,
            float(self.get_parameter('profile_moving_linear_threshold').value),
        )
        self.profile_moving_angular_threshold = max(
            0.001,
            float(self.get_parameter('profile_moving_angular_threshold').value),
        )
        ...
        # One warm client can safely own multiple goals and avoids five extra
        # action waitables on every executor wakeup.
        self.recovery_plan_client = self.preview_client
        ...
        self.periodic_deadlines = {
            name: periodic_start + interval
    ...
    def process_periodic_work(self) -> None:
        """Run periodic work at its original cadence from one ROS timer."""
        now = time.monotonic()

        def due(name: str) -> bool:
            if now < self.periodic_deadlines[name]:
                return False
            self.periodic_deadlines[name] = (
                now + self.periodic_intervals[name]
            )
            return True

        if due('mapping_deadman'):
    ...
    def agent_readiness_snapshot(self) -> dict[str, Any]:
        ...
        robot_moving = self.velocity_indicates_motion(
            velocity,
            linear_threshold=self.profile_moving_linear_threshold,
            angular_threshold=self.profile_moving_angular_threshold,
        )
        scan_age = (
    ...
    @staticmethod
    def velocity_indicates_motion(
        velocity: dict[str, float] | None,
        *,
        linear_threshold: float,
        angular_threshold: float,
    ) -> bool:
        """Reject stationary EKF noise without masking deliberate motion."""
        if not velocity:
            return False
        return (
            abs(float(velocity.get('linear_velocity', 0.0)))
            > linear_threshold
            or abs(float(velocity.get('angular_velocity', 0.0)))
            > angular_threshold
        )
    ...
    async def handle_message(
        ...
        if message_type == 'connection_ack':
            ...
            if message.get('action'):
                await self.handle_robot_operation(websocket, message)
            else:
                await self.queue_navigation_command(websocket, message)
    ...
    async def handle_robot_operation(
        self,
        websocket: Any,
        message: dict[str, Any],
    ) -> None:
        ...
        valid = (
            isinstance(message.get('command_id'), str)
            and bool(message.get('command_id'))
            and message.get('robot_id') == self.robot_id
            and action in supported
            and isinstance(message.get('payload', {}), dict)
        )
        with self.robot_operation_lock:
            busy = self.robot_operation_active
            if valid and rejection is None and not busy:
                self.robot_operation_active = True
        if not valid or rejection is not None or busy:
            detail = rejection or (
                'Another robot operation is already running'
                if busy else 'Invalid robot operation command'
            )
            await self.send_json(
                websocket,
                self.command_status_payload(message, 'rejected', detail),
            )
            return
        ...
        threading.Thread(
            target=self.run_robot_operation,
            args=(dict(message),),
            name=f'robot-operation-{action}',
            daemon=True,
        ).start()

    def run_robot_operation(self, command: dict[str, Any]) -> None:
        action = str(command['action'])
        self.send_command_status(command, 'started', f'Starting {action}')
        try:
            if action == 'navigation.recover':
                ...
                if manager_available:
                    succeeded = False
                    detail = (
                        'Nav2 lifecycle server is available; restart was refused'
                    )
                else:
                    self.cancel_active_navigation_for_operation(resume=False)
                    succeeded, output = self.navigation_recovery_runner.system_action(
                        'restart-navigation'
                    )
                    detail = output or (
                        'Nav2 restarted; set Initial Pose before navigation'
                        if succeeded else lifecycle_detail
                    )
            elif action == 'system.start_navigation':
                succeeded, output = self.navigation_recovery_runner.system_action(
                    'start-navigation'
                )
                detail = output or (
                    'Robot ROS and Nav2 stack start requested; set Initial Pose '
                    'if localization is not ready'
                )
            elif action == 'system.stop_navigation':
                self.cancel_active_navigation_for_operation(resume=False)
                succeeded, output = self.navigation_recovery_runner.system_action(
                    'stop-navigation'
                )
                detail = output or 'Robot ROS and Nav2 stack stopped; Agent remains online'
            else:
                self.cancel_active_navigation_for_operation(resume=False)
                succeeded, output = self.navigation_recovery_runner.system_action(
                    'poweroff'
                )
                detail = output or 'ODROID shutdown scheduled'
        except Exception as error:
            succeeded, detail = False, f'Robot operation failed: {error}'
        finally:
            with self.robot_operation_lock:
                self.robot_operation_active = False
    ...
    def cancel_active_navigation_for_operation(
        self,
        *,
        resume: bool,
    ) -> dict[str, Any] | None:
        ...
        with self.command_lock:
            ...
            goal_handle = self.active_goal_handle
            if active is not None and resume:
                self.recovery_cancelled_command_ids.add(str(active['command_id']))
        if goal_handle is not None:
            ...
            with self.command_lock:
                self.recovery_cancelled_command_ids.discard(
                    str(active['command_id'])
                )
        if active is not None:
            self.clear_active_command(str(active['command_id']))
            deadline = time.monotonic() + 4.0
            while time.monotonic() < deadline:
                with self.command_lock:
                    pending_result = (
                        str(active['command_id'])
                        in self.recovery_cancelled_command_ids
                    )
    ...
    def stall_escape_decision(
        ...
        *,
        ...
        return choose_stall_escape(
            ...
            rear_required=(math.inf if prefer_turn else 0.45),
        )

    def stall_turn_decisions(self) -> tuple[StallEscapeDecision, ...]:
    ...
    def stall_arc_decisions(self) -> tuple[StallEscapeDecision, ...]:
    ...
    def _service_response(
        ...
        *,
        ...
        """Wait for a pre-created ROS service client without spinning this node."""
        ...
        def done_callback(response_future: Any) -> None:
            try:
                result['response'] = response_future.result()
            except Exception as error:  # ROS future errors become a safe refusal.
        ...
        if not completed.wait(timeout_seconds):
            return False, f'service response timed out after {timeout_seconds:.1f}s'
        if result.get('error'):
    ...
    def guarded_arc_for_recovery(self, yaw: float) -> tuple[bool, str]:
        """Run a scan/odometry-guarded rolling arc outside Nav2 planning."""
        direction = 1 if float(yaw) > 0.0 else -1
    ...
    def recovery_motion_is_safe(self) -> tuple[bool, str]:
        """Revalidate hard safety gates before each additional maneuver."""
        if self.motion_stop_latched():
        ...
        return True, 'recovery motion safety checks passed'

    def wait_for_recovery_motion_safe(
        self,
        timeout_seconds: float = 4.0,
        poll_seconds: float = 0.10,
    ) -> tuple[bool, str]:
        """Let odometry and AMCL settle after a bounded recovery motion."""
        deadline = time.monotonic() + max(0.1, float(timeout_seconds))
        detail = 'recovery motion safety state is unavailable'
        while time.monotonic() < deadline:
            safe, detail = self.recovery_motion_is_safe()
            if safe:
                return True, detail
            # E-stop and motor diagnostics are hard gates. Only localization
            # freshness is allowed to converge after a completed maneuver.
            if not detail.startswith('localization is not ready'):
                return False, detail
            time.sleep(max(0.0, float(poll_seconds)))
        return False, f'{detail}; did not recover within {timeout_seconds:.1f}s'

    def run_alternative_escape_maneuvers(
        self,
        command_id: str,
        *,
        attempted_turn_values: set[float] | None = None,
    ) -> tuple[bool, str]:
        """Try a bounded reverse-arc sequence before replanning once."""
        attempts: list[str] = []
        tried_values = set(attempted_turn_values or ())
        arc_options = tuple(
            option
            for option in self.stall_arc_decisions()
            if float(option.value) not in tried_values
        )
        if arc_options:
            preferred = arc_options[0]
            tried_values.add(float(preferred.value))
            safe, safety_detail = self.wait_for_recovery_motion_safe()
            if not safe:
                return False, safety_detail
            turned, output = self.guarded_arc_for_recovery(preferred.value)
            attempts.append(
                f'preferred rolling arc ({preferred.detail}): '
                f'{output or "no response"}'
            )
            if turned:
                return True, '; '.join(attempts)

        # A footprint-safe spin can still be impossible while a chassis corner
        # is close to furniture. Move the center of rotation another 0.10 m
        # rearward, but only when a fresh scan independently authorizes it.
        reverse = self.stall_escape_decision()
        reversed_again = False
        if reverse.action == 'back_up':
            safe, safety_detail = self.wait_for_recovery_motion_safe()
            if not safe:
                return False, safety_detail
            reversed_again, output = self.guarded_reverse_for_recovery()
            attempts.append(
                f'additional guarded reverse ({reverse.detail}): '
                f'{output or "no response"}'
            )
            if reversed_again:
                ...
                # The center of rotation moved, so a direction rejected at
                # the old pose is a genuinely new collision-checked attempt.
                tried_values.clear()

        # Re-read LaserScan after the center of rotation changes. Try the
        # clearest rolling direction, then the other scan-safe direction.
        fresh_options = self.stall_arc_decisions()
        for option in fresh_options[:2]:
            ...
            if not safe:
                return False, safety_detail
            turned, output = self.guarded_arc_for_recovery(value)
            direction = 'left' if value > 0.0 else 'right'
            attempts.append(
                f'{direction} rolling arc ({option.detail}): '
                f'{output or "no response"}'
            )
            if turned:
                return True, '; '.join(attempts)
        ...
        if not attempts:
            attempts.append('no fresh scan-safe turn or additional reverse was available')
        self.get_logger().warning(
            f'Alternative escape maneuvers for {command_id} ended without a '
            f'completed turn: {"; ".join(attempts)}'
        )
        # The first guarded reverse may have changed the pose enough to plan a
        # route, but only continue if the last failed maneuver did not leave
        # the base in PRE_STALL/FAULTED. Previously this returned success
        # unconditionally and could requeue a Nav2 goal while motion was
        # inhibited by the base driver.
        safe, safety_detail = self.wait_for_recovery_motion_safe()
        if not safe:
            return False, (
                f'{"; ".join(attempts)}; recovery stopped because '
                f'{safety_detail}'
            )
        attempts.append(
            'no turn completed; continuing from the verified-safe reverse pose'
        )
        return True, '; '.join(attempts)
    ...
    def recover_navigation(self, operation: dict[str, Any]) -> tuple[bool, str]:
        ...
        if mapping_phase != 'IDLE':
            return False, 'No connected path: recovery is unavailable during mapping'
        ...
        if not isinstance(target, dict):
            return False, 'No connected path: no current navigation goal is available'
        ...
        if not lifecycle_ok:
            return False, (
                ...
                'use the Admin restart control only if the lifecycle server is broken. '
                f'{lifecycle_detail}'
            )
        ...
        if active is not None:
            # Re-run the same delivery stage after the one explicit planning
            # check succeeds. The task identity remains unchanged.
            self.command_queue.put(active)
        return True, 'Navigation recovered: costmaps cleared and route recomputed'

    def maybe_start_blocked_pose_recovery(
        self,
        command: dict[str, Any],
        original_detail: str,
    ) -> bool:
        """Try one scan-guarded reverse or rolling arc after an early abort."""
        command_id = str(command.get('command_id', 'unknown'))

        def refuse(reason: str) -> bool:
            self.get_logger().warning(
                f'Blocked-pose recovery not started for {command_id}: {reason}'
            )
            return False

        if self.motion_stop_latched():
            return refuse('an emergency stop is latched')
        started = float(command.get('_navigation_started_monotonic', 0.0))
        elapsed = time.monotonic() - started if started > 0.0 else math.inf
        if started <= 0.0 or elapsed > self.blocked_start_recovery_window_seconds:
            return refuse(
                'navigation elapsed time '
                f'{elapsed:.1f}s is outside the '
                f'{self.blocked_start_recovery_window_seconds:.1f}s window'
            )
        escape = self.stall_escape_decision()
        if escape.action not in {'back_up', 'arc', 'spin'}:
            return refuse(f'LaserScan found no safe escape: {escape.detail}')
        fault = self.motor_fault_snapshot()
        if fault is None or not fault.get('motors_enabled'):
        ...
        if not command_id:
            return refuse('command ID is missing')
        with self.blocked_start_recovery_lock:
            if (
                self.blocked_start_recovery_active
                or command_id in self.blocked_start_recovery_attempted_ids
            ):
                return refuse('this command was already attempted or recovery is busy')
            with self.robot_operation_lock:
                if self.robot_operation_active:
                    return refuse('another robot operation is active')
                self.robot_operation_active = True
            if len(self.blocked_start_recovery_attempted_ids) >= 512:
                self.blocked_start_recovery_attempted_ids.clear()
            self.blocked_start_recovery_active = True
            self.blocked_start_recovery_attempted_ids.add(command_id)

        self.clear_active_command(command_id)
        threading.Thread(
            target=self.run_blocked_pose_recovery,
            args=(
                dict(command),
                original_detail,
                escape.action,
            ...
            name=f'blocked-pose-recovery-{command_id}',
            daemon=True,
        ).start()
        return True
    ...
    def run_blocked_pose_recovery(
        ...
        command_id = str(command.get('command_id', ''))
        try:
            if escape_action == 'arc':
                direction = 'left' if escape_value > 0.0 else 'right'
                maneuver = f'guarded rolling {direction} arc'
                self.send_command_status(
                    command,
                    'started',
                    f'Nav2 reported a blocked start; checking a {direction} arc',
                )
                escaped, escape_output = self.guarded_arc_for_recovery(
                    escape_value
                )
            elif escape_action == 'spin':
                direction = 'left' if escape_value > 0.0 else 'right'
                maneuver = f'collision-checked {direction} turn'
                self.send_command_status(
                    command,
                    'started',
                    f'Nav2 reported a blocked start; checking a {direction} turn',
                )
                escaped, escape_output = self.spin_for_recovery(escape_value)
            else:
                maneuver = 'guarded reverse'
                self.send_command_status(
                    command,
                    'started',
            ...
            if not escaped:
                detail = (
                    f'{original_detail}; {maneuver} was stopped or refused '
                    f'({scan_detail}): {escape_output or "no response"}'
                )
                self.get_logger().error(
            ...
            if not lifecycle_ok:
                detail = (
                    f'{maneuver.capitalize()} completed, but Nav2 did not return to an '
            ...
            if escape_action == 'back_up':
                maneuver_ok, maneuver_detail = (
                    self.run_alternative_escape_maneuvers(command_id)
                )
            else:
                maneuver_ok = True
                maneuver_detail = (
                    f'{maneuver} ({scan_detail}): '
                    f'{escape_output or "completed"}'
                )
            if not maneuver_ok:
                detail = (
                    'Guarded reverse completed, but alternative escape motion '
                    f'was stopped: {maneuver_detail}'
                )
                self.get_logger().error(
                    f'Blocked-pose recovery for {command_id} failed: {detail}'
                )
                ...
                return
            self.get_logger().warning(
                f'Blocked-pose recovery maneuvers for {command_id}: '
                f'{maneuver_detail}'
            )
            ...
            if not cleared:
                detail = (
                    f'{maneuver.capitalize()} completed, but Nav2 costmaps could not be '
                    f'cleared: {clear_output or "no response"}'
                )
                self.get_logger().error(
                    f'Blocked-pose recovery for {command_id} failed after reverse: '
                    f'{detail}'
                )
                self.send_navigation_result(command, 'aborted', detail[:500])
                return
            ...
            if not planned:
                failure = classify_plan_failure(plan_output)
                detail = (
                    f'{maneuver.capitalize()} completed, but the route is still '
                    'unavailable: '
                    f'{failure}: {plan_output or "planner returned no path"}'
                )
                self.get_logger().error(
                    f'Blocked-pose recovery for {command_id} failed after reverse: '
                    f'{detail}'
            ...
            command['_navigation_started_monotonic'] = 0.0
            self.command_queue.put(command)
            self.get_logger().warning(
                f'Blocked-pose recovery for {command_id} completed; retrying goal once'
            )
        except Exception as error:
            detail = f'Blocked-pose recovery failed: {error}'
            self.get_logger().error(f'{command_id}: {detail}')
            self.send_navigation_result(command, 'aborted', detail[:500])
        finally:
            with self.blocked_start_recovery_lock:
                self.blocked_start_recovery_active = False
    ...
    def navigation_result_callback(
        ...
        with self.command_lock:
            recovery_cancelled = command_id in self.recovery_cancelled_command_ids
            if recovery_cancelled:
                self.recovery_cancelled_command_ids.discard(command_id)
        if recovery_cancelled:
            self.get_logger().info(
                f'Suppressed navigation cancellation result during recovery: {command_id}'
            )
            self.clear_active_command(command_id)
            return
    ...
    def destroy_node(self) -> bool:
        ...
        self.destroy_timer(self.periodic_timer)
